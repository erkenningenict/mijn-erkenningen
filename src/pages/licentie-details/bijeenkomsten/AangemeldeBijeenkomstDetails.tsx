...
const AangemeldeBijeenkomstDetailsPage: React.FC<
  AangemeldeBijeenkomstDetailsProps
> = () => {
  ...
  if (!cursus) {
    ...
    return <Redirect to={`/mijn-licenties/${certificeringId}/bijeenkomsten`} />;
    // return <ErrorComponent error={error} />;
  }
      ...
      const res = await unRegisterCourse({
        variables: {
          input: {
            ...
            dateTime: toDutchTime(firstSessie(cursus?.Cursus)?.DatumBegintijd),
          },
        },
      });
        ...
        <IonCard className="welcome-card">
          ...
          <IonCardContent>
            <dl className="styled-dl">
              ...
              <dt>Prijs</dt>
              <dd>{toDutchMoney(cursus.Cursus.Prijs, { euroPrefix: true })}</dd>
